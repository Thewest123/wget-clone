...
std::string receive_http_message(BIO *bio)
{
    ...
    // {
    //     std::string chunkSizeStr = body.substr(0, body.find("\r\n"));
        
    //     std::stringstream ss;
    //     ss << std::hex << chunkSizeStr;
    //     ss >> chunkSize;

    //     std::cout << "--- substr(): " << chunkSizeStr << std::endl;
    //     std::cout << "--- substrsize(): " << chunkSize << std::endl;
    // }

    std::string newData;
    while ((newData = my::receive_some_data(bio)).length() > 0) {
        
        // std::stringstream ss;
        // std::cout << "--- substr(): " << newData.substr(0, newData.find('\n')) << std::endl;
        // ss << std::hex << newData.substr(0, newData.find('\n'));
        // ss >> content_length;
    ...
    // while (true)
    // {
    //     std::string newData = my::receive_some_data(bio);
    //     body += newData;

    //     if (body.size() < chunkSize)
    //         continue;

    //     newData = my::receive_some_data(bio);
    //     body += newData;
        
    //     std::stringstream ss;
    //     std::string chunkSizeStr = newData.substr(0, newData.find("\r\n"));
    //     ss << std::hex << chunkSizeStr;
    //     ss >> chunkSize;
...
int main()
{
    ...
    SSL_CTX_load_verify_locations(ctx.get(), NULL, "./keys");

    auto bio = my::UniquePtr<BIO>(BIO_new_connect("www.tenskytarou.cz"));
    if (bio == nullptr) {
    ...
    SSL_set_tlsext_host_name(my::get_ssl(ssl_bio.get()), "www.tenskytarou.cz");
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
    SSL_set1_host(my::get_ssl(ssl_bio.get()), "www.tenskytarou.cz");
#endif
    if (BIO_do_handshake(ssl_bio.get()) <= 0) {
        my::print_errors_and_exit("Error in BIO_do_handshake");
    }
    
    my::verify_the_certificate(my::get_ssl(ssl_bio.get()), "www.tenskytarou.cz");

    my::send_http_request(ssl_bio.get(), "GET / HTTP/1.0", "www.tenskytarou.cz");
